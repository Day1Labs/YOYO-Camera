...
extension Pager.PagerContent {
    ...
    var offsetLowerbound: CGFloat {
        guard currentPage == 0, !isInifinitePager else { return CGFloat(numberOfPages) * size.width }
    ...
    var offsetUpperbound: CGFloat {
        guard currentPage == numberOfPages - 1, !isInifinitePager else { return -CGFloat(numberOfPages) * size.width }
    ...
    var alignmentOffset: CGFloat {
        ...
        case let (.end(insets), _), (.justified(let insets), dataDisplayed.count - 1):
            if isVertical {
    ...
    func opacity(for item: PageWrapper<Element, ID>) -> Double {
        guard let opacityIncrement else { return 1 }
