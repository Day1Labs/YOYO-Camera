        ...
        var body: some View {
            let stack = HStack(spacing: interactiveItemSpacing) {
                ForEach(dataDisplayed, id: id) { item in
                    Group {
                        if isInifinitePager, isEdgePage(item) {
                            EmptyView()
                        } else {
                            content(item.element)
                                ...
                                .rotation3DEffect((isHorizontal ? .zero : Angle(degrees: -90)) - scrollDirectionAngle,
                                                  axis: (0, 0, 1))
                                .rotation3DEffect(angle(for: item),
                                                  axis: axis)
            ...
            .frame(size: size)
                ...
                let wrappedView = stack
                    .focusable()
                    .onMoveCommand(perform: onMoveCommandSent)
            #endif
...
extension Pager.PagerContent {
    ...
    // `DragGesture` customized to work with `Pager`
        ...
        func onDragGestureEnded() {
            ...
            let pagingAnimation = pagingAnimation?((page, newPage, draggingOffset, draggingVelocity)) ?? defaultPagingAnimation
        ...
        var dragResult: (page: Int, increment: Int) {
            let currentPage = currentPage(sensitivity: sensitivity.value)
            let velocity = -draggingVelocity * (Locale.current.isRightToLeft ? -1 : 1)

            guard allowsMultiplePagination else {
                var newPage = currentPage
                if currentPage == page, abs(velocity) > 500 {
                    if isInifinitePager {
                        newPage = (newPage + Int(velocity / abs(velocity)) + numberOfPages) % numberOfPages
                    } else {
            ...
            let velocityPageIncrement = Int(CGFloat(abs(velocity)) / (side / pageDistance) / 500)

            var offsetPageIncrement = direction == .forward ? currentPage - page : page - currentPage
            if isInifinitePager {
                offsetPageIncrement = (offsetPageIncrement + numberOfPages) % numberOfPages
            }

            let pageIncrement = min(velocityPageIncrement + offsetPageIncrement, maxIncrement)
            var newPage = direction == .forward ? page + pageIncrement : page - pageIncrement
            if isInifinitePager {
                newPage = (newPage + numberOfPages) % numberOfPages
            }
